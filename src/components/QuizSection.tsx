      ...
      case 'topic_selection':
        return (
          <div className="flex flex-col items-center gap-4 p-8">
            <h3 className="text-xl font-semibold">What would you like to be quizzed on?</h3>
            <div className="flex w-full max-w-sm items-center space-x-2">
              <Input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="e.g., 'Sustainable Tourism'" />
      ...
      case 'in_progress':
        return (
          <div className="space-y-6">
            ...
            <Button onClick={handleSubmitQuiz} className="w-full">Submit</Button>
          </div>
        );
      ...
      case 'results':
        return (
          <div className="text-center p-4">
            ...
            <p className="text-4xl my-4 font-bold">{results?.score} / {questions.length}</p>
            <div className="text-left space-y-4 my-6">
              {questions.map((q, index) => (
                  <div key={index} className={`p-3 rounded-md ${results?.results[index].isCorrect ? 'bg-green-100 dark:bg-green-900' : 'bg-red-100 dark:bg-red-900'}`}>
                      <p className="font-semibold">{index + 1}. {q.questionText}</p>
                      <p className={`text-sm ${results?.results[index].isCorrect ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}>
                        Your answer: {q.options[userAnswers[index]!]} {results?.results[index].isCorrect ? ' (Correct)' : ' (Incorrect)'}
                      </p>
                      {!results?.results[index].isCorrect && (
                          <p className="text-sm text-green-700 dark:text-green-300">Correct answer: {q.options[results?.results[index].correctAnswerIndex!]}</p>
                      )}
                  </div>
              ))}
            </div>
    ...
    <section id="quiz" className="max-w-4xl mx-auto py-10 px-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Test your knowledge on any eco-topic and see how you score!</CardDescription>
        </CardHeader>
        <CardContent>
          {renderContent()}
        </CardContent>
