...
const DestinationRegistration = () => {
  ...
  const [permitIds, setPermitIds] = useState<string[]>([]); // We track IDs to associate them later
  ...
  const handleCreateOrUpdateDestination = async () => {
    ...
    try {
      ...
      if (destinationId) {
        ...
        // Insert new record and get the ID back
      ...
      // Associate any freshly uploaded permits with the destination ID
      if (permitIds.length > 0 && finalDestinationId) {
        ...
        setPermitIds([]); // Clear the temporary permit IDs after associating them
      }
  ...
  const handlePermitsUploaded = (uploadedPermits: any[]) => {
      // Collect the IDs of newly uploaded permits to be associated later
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-20 pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12"><h1 className="text-4xl font-bold text-forest mb-4">Join EcoLakbay as a Partner Destination</h1><p className="text-lg text-muted-foreground max-w-2xl mx-auto">Register your eco-friendly establishment to become part of the sustainable tourism movement in the Philippines.</p></div>
          <div className="grid md:grid-cols-3 gap-6 mb-12"><Card className="text-center"><CardHeader><Star className="w-8 h-8 text-accent mx-auto mb-2" /><CardTitle className="text-lg">Increased Visibility</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">Reach eco-conscious travelers actively seeking sustainable destinations.</p></CardContent></Card><Card className="text-center"><CardHeader><Building className="w-8 h-8 text-accent mx-auto mb-2" /><CardTitle className="text-lg">Community Support</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">Join a network of like-minded businesses committed to sustainable tourism.</p></CardContent></Card><Card className="text-center"><CardHeader><Camera className="w-8 h-8 text-accent mx-auto mb-2" /><CardTitle className="text-lg">Marketing Tools</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">Access professional listing features and promotional opportunities.</p></CardContent></Card></div>
          <Card>
            ...
            <CardContent>
              <Tabs value={currentTab} onValueChange={setCurrentTab} className="space-y-6">
                ...
                <TabsContent value="business" className="space-y-6">
                    <div className="space-y-4"><h3 className="text-lg font-semibold text-forest">Business Information</h3><div className="grid md:grid-cols-2 gap-4"><div className="space-y-2"><Label htmlFor="businessName">Business Name *</Label><Input id="businessName" placeholder="Enter your business name" value={formData.businessName} onChange={(e) => handleInputChange("businessName", e.target.value)} required /></div><div className="space-y-2"><Label htmlFor="businessType">Business Type *</Label><Select value={formData.businessType} onValueChange={(value) => handleInputChange("businessType", value)}><SelectTrigger><SelectValue placeholder="Select business type" /></SelectTrigger><SelectContent><SelectItem value="hotel">Hotel/Resort</SelectItem><SelectItem value="restaurant">Restaurant/Café</SelectItem><SelectItem value="attraction">Tourist Attraction</SelectItem><SelectItem value="other">Other</SelectItem></SelectContent></Select></Select></div></div><div className="space-y-2"><Label htmlFor="description">Business Description *</Label><Textarea id="description" placeholder="Describe your business, services, and what makes you special..." className="min-h-[120px]" value={formData.description} onChange={(e) => handleInputChange("description", e.target.value)} required /></div></div>
                    ...
                    <div className="space-y-4"><h3 className="text-lg font-semibold text-forest">Sustainability Practices</h3><div className="space-y-2"><Label htmlFor="sustainabilityPractices">Tell us about your environmental initiatives</Label><Textarea id="sustainabilityPractices" placeholder="Describe your eco-friendly practices, community involvement, waste reduction efforts, etc." className="min-h-[120px]" value={formData.sustainabilityPractices} onChange={(e) => handleInputChange("sustainabilityPractices", e.target.value)} /></div></div>
                ...
                <TabsContent value="photos" className="space-y-6">
                    <CardTitle>Destination Photos</CardTitle><CardDescription>Upload at least one high-quality photo. The first image uploaded will be your main cover photo.</CardDescription><PhotoUpload onPhotosChange={handlePhotosChange} bucketName="destination-photos"/><div className="flex justify-between pt-6"><Button variant="outline" onClick={() => setCurrentTab("business")}>Back to Business Info</Button><Button variant="eco" onClick={() => handleProceed('verification')} disabled={isSubmitting || photoUrls.length === 0}>{isSubmitting ? 'Saving...' : 'Save & Proceed to Verification'}</Button></div>
                </TabsContent>
                <TabsContent value="verification" className="space-y-6">
                  {user && destinationId && (<PermitUpload userId={user.id} destinationId={destinationId} onPermitsUploaded={handlePermitsUploaded} />)}
                  <div className="flex justify-between pt-6"><Button variant="outline" onClick={() => setCurrentTab("photos")}>Back to Photos</Button><Button variant="eco" onClick={() => handleProceed('review')} disabled={isSubmitting}>{isSubmitting ? 'Saving...' : 'Save & Proceed to Review'}</Button></div>
                </TabsContent>
                <TabsContent value="review" className="space-y-6">
                  <h3 className="text-lg font-semibold">Application Summary</h3><Card><CardHeader><CardTitle className="text-base">Review Your Information</CardTitle></CardHeader><CardContent><p className="text-muted-foreground">All required information has been provided. Click the button below to submit your application for review by our team.</p></CardContent></Card>
                  <div className="flex justify-between pt-6"><Button variant="outline" onClick={() => setCurrentTab("verification")}>Back</Button><Button variant="eco" size="lg" onClick={handleFinalSubmit} disabled={isSubmitting}>{isSubmitting ? 'Submitting...' : 'Submit Full Application'}</Button></div>
                </TabsContent>
              </Tabs>
            </CardContent>
          </Card>
          <div className="mt-12 text-center"><Card className="bg-muted/50"><CardContent className="pt-6"><h3 className="text-lg font-semibold text-forest mb-2">What Happens Next?</h3><p className="text-muted-foreground">After submitting your registration, our team will review your application within 5-7 business days. We may contact you for additional information or to schedule an interview about your sustainability practices.</p></CardContent></Card></div>
        </div>
      </main>
      <Footer />
    </div>
