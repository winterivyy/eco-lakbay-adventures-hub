...
const Community = () => {
  ...
  const [profile, setProfile] = useState<any>(null); // For current user info
  ...
  useEffect(() => { fetchPostsAndProfiles(); }, [user]);

  const handleCreatePost = () => {
    if (!user) { toast({ title: "Authentication required", description: "Please sign in to create a post.", variant: "destructive" }); return; }
  ...
  const handleLike = async (postId: string) => {
    ...
    setPosts(posts.map(p => p.id === postId ? { ...p, userLiked: !p.userLiked, likes_count: p.userLiked ? p.likes_count - 1 : (p.likes_count || 0) + 1 } : p));
    try {
      const post = originalPosts.find(p => p.id === postId);
      if (post?.userLiked) { await supabase.from('post_likes').delete().match({ post_id: postId, user_id: user.id }); }
      else { await supabase.from('post_likes').insert({ post_id: postId, user_id: user.id }); }
    } catch (error) { console.error('Error liking post:', error); setPosts(originalPosts); }
  };
  ...
  const handleAddComment = async (postId: string) => {
    ...
    const tempComment = { id: Math.random(), content, created_at: new Date().toISOString(), profiles: { full_name: profile?.full_name || 'You', avatar_url: profile?.avatar_url } };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              {isAdmin && (
                <Card className="mb-6 border-amber-200 bg-amber-50 dark:bg-amber-950">
                  <CardHeader><div className="flex items-center gap-2"><Shield className="w-5 h-5 text-amber-600" /><CardTitle className="text-amber-800 dark:text-amber-200">Admin Panel</CardTitle></div></CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div className="text-center"><Users className="w-8 h-8 mx-auto text-blue-600 mb-2" /><p className="text-sm font-medium">Top Users</p><p className="text-2xl font-bold text-blue-600">{profiles.length}</p></div>
              ...
              <div className="space-y-6">
                {loading ? (<p className="text-center py-8 text-muted-foreground">Loading community feed...</p>) : posts.length === 0 ? (
                  <Card className="text-center py-12"><CardContent><p className="text-muted-foreground">No posts yet. Be the first to share your story!</p><Button onClick={handleCreatePost} className="mt-4">Create First Post</Button></CardContent></Card>
                ) : (
                  posts.map((post) => (
            ...
            <div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">{profiles.map((profile, index) => (<div key={index} className="flex items-center justify-between"><div className="flex items-center space-x-2"><span className="text-lg">{index === 0 ? "🏆" : index === 1 ? "🥈" : "🥉"}</span><span className="text-sm font-medium">{profile.full_name || "Anonymous User"}</span></div><span className="text-xs text-amber font-medium">{profile.points || 0} pts</span></div>))}
                  {profiles.length === 0 && (<p className="text-sm text-muted-foreground text-center py-4">No champions yet. Start posting!</p>)}</div>
